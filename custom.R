      ...
      if (!isTRUE(data_missing (id))) {
        if (isTRUE(nchar(id) > 6L)) {
          if (isTRUE(!as.integer(substr(id, 1L, 2L)) %in% hosp_to_validate)) {
            form__x2014cin = c(form__x2014cin, "Biodata")
            name__x2014cin = c(name__x2014cin, "")
...
if(is.na(t_birth) | (!is.na(t_birth) & as.character(t_birth)!='-1'  &
                     (!grepl('[0-9]:[0-9]',t_birth) | (!grepl('am$',tolower(t_birth)) & !grepl('pm$',tolower(t_birth)))))){
  form__x2014cin = c(form__x2014cin, "Biodata")
...
if(is.na(t_adm) | (!is.na(t_adm) & as.character(t_adm)!='-1'  &
                   (!grepl('[0-9]:[0-9]',t_adm) | (!grepl('pm$',tolower(t_adm)) & !grepl('pm$',tolower(t_adm)))))){
  form__x2014cin = c(form__x2014cin, "Biodata")
...
if(!is.na(cpap) & as.numeric(cpap)==1){
  if(is.na(cpap_start_time) | (!is.na(cpap_start_time) & as.character(cpap_start_time)!='-1'  &
                               (!grepl('[0-9]:[0-9]',cpap_start_time) | (!grepl('am$',tolower(cpap_start_time)) & !grepl('pm$',tolower(cpap_start_time)))))){
    form__x2014cin = c(form__x2014cin, "Biodata")
  ...
  if(is.na(cpap_end_time) | (!is.na(cpap_end_time) & as.character(cpap_end_time)!='-1'  &
                             (!grepl('[0-9]:[0-9]',cpap_end_time) | (!grepl('am$',tolower(cpap_end_time)) & !grepl('pm$',tolower(cpap_end_time)))))){
    form__x2014cin = c(form__x2014cin, "Biodata")
...
if(is.na(t_discharge) | (!is.na(t_discharge) & as.character(t_discharge)!='-1'  &
                         (!grepl('[0-9]:[0-9]',t_discharge) | (!grepl('am$',tolower(t_discharge)) & !grepl('pm$',tolower(t_discharge)))))){
  form__x2014cin = c(form__x2014cin, "Biodata")
